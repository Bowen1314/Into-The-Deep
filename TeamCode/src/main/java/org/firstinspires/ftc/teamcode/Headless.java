...
@TeleOp(name = "AAA - Driving System", group = "AAA-First")
public class Headless extends OpMode {
    ...
    public void loop() {
        ...
        if (gamepad1.left_bumper && !leftbumper_Pressed) {
            dk = !dk;
            if (dk) {
                RD.setTargetPosition(100);
                LD.setTargetPosition(100);
            } else {
                RD.setTargetPosition(0);
                LD.setTargetPosition(0);
            }
            RD.setPower(.5);
            LD.setPower(.5);
            leftbumper_Pressed = true;
        }
        if (!gamepad1.left_bumper) {
            leftbumper_Pressed = false;
        }
        //end of level system
    }
}